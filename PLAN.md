...
## 6.220 — Форвард 2.0, Phase 2+3: picker tap=preview vs Select=batch, pending-forward composer banner — 2026-09-05

Продолжение §6.219: реализованы два оставшихся пункта мастер-плана
"Форвард 2.0", которые Александр явно выделил отдельно после гринлайта
остального ("должен быть момент, что ты типа когда пересылаешь и
открываешь чат, и там тоже сверху это появляется в композере, типа в
input field'е... У нас в нашем приложении оно уже построено, а нам
тоже надо такой preview").

1. **Пикер (Phase 2)** — `components/chat/forward-picker-modal.tsx`
   получил режим `mode: "tap" | "select"` (переключатель "Вибрати" /
   "Скасувати" в шапке, по умолчанию `"tap"`, как у Telegram):
   - `"tap"` — обычный клик по строке чата сразу вызывает новый
     обязательный проп `onPickSingle(chatId)`. Ни чекбоксов, ни кнопки
     "Переслати" в этом режиме нет вообще — вся судьба выбора отдана
     вызывающей стороне.
   - `"select"` — старое поведение без изменений: чекбоксы +
     немедленная batch-отправка на все выбранные чаты по кнопке снизу,
     без шага превью. Остаётся правильным инструментом для "разослать
     10 людям сразу".

2. **Pending-forward hold (Phase 3)** — новый `lib/forward-pending-
   hold.ts`: module-level переменная (не React Context — значение
   живёт ровно одну навигацию "закрыть пикер -> замаунтить чат-
   получатель" и читается максимум одним компонентом), зеркалит
   мобильный `ForwardPendingHold`. `putForwardPending`/
   `takeForwardPendingFor(chatId)`/`clearForwardPending` — забор
   строго по `targetChatId`, чтобы чужой форвард не утёк в третий чат,
   если пользователь передумал по дороге.

3. **`onPickSingle` в page.tsx** — строит `ForwardPendingDraft`
   (`messages` = тот же `forwardSource`, oldest-first; `ownerLabel` —
   имя оригинального автора ПОСЛЕДНЕГО сообщения, через
   `resolveForwardOwnerLabel`, с тем же fallback-chain на
   `forwardFrom.user ?? fromId`, что и у `forwardToOneChat`). Дальше
   развилка:
   - целевой чат == текущий (`chatId`) — кладёт draft в `pendingForward`
     напрямую, без навигации (`router.push` на тот же route всё равно
     не перемонтирует компонент, so mount-эффект никогда бы не сработал
     для этого случая);
   - другой чат — `putForwardPending(draft)` + `router.push(/chats/
     ${targetChatId})`; новый mount-эффект (`useEffect(() => {...},
     [])`, один раз на маунт — этот `[chatId]`-route всегда
     перемонтирует компонент целиком при навигации) вызывает
     `takeForwardPendingFor(chatId)` и, если что-то нашлось,
     проставляет `pendingForward`.

4. **`ForwardComposeBar`** (`components/chat/message-actions-menu.tsx`)
   — новый компонент, 1:1 по форме с `EditComposeBar`/`ReplyComposeBar`
   (акцентная полоса + заголовок + превью-строка + крестик), рендерится
   `inline` прямо ВНУТРИ той же композер-пилюли, над textarea — именно
   "сверху в input field'е", как просил Александр. Заголовок — "Переслати
   N повідомлень" (число всегда интерполируется как есть, без
   пословной грамматики множественного числа — та же договорённость,
   что уже используется в `selectionDeleteConfirm`'а "Видалити N
   повідомлень?"), под ним — `ownerLabel`.

5. **`sendPendingForwardBatch()`** — срабатывает из early-branch в
   `send()` (`if (pendingForward && !overrideText && !meet)`, тем же
   способом, что и `editingMessage`-ветка чуть выше). Копирует ТОЧНУЮ
   семантику caption из мобильного `forward_multi_send.dart`'s
   `_sendOne()` (прочитан полностью в предыдущем сегменте):
   - все сообщения, кроме последнего, пересылаются как есть, без
     caption;
   - если ПОСЛЕДНЕЕ сообщение содержит медиа (documents/contacts) и
     caption непустой — caption ЗАМЕНЯЕТ текст/entities этого
     сообщения в ТОМ ЖЕ send-вызове (новый опциональный параметр
     `captionOverride` у `forwardToOneChat`, `?? ""` намеренно НЕ
     используется — пустая строка не должна затирать оригинальный
     caption медиа-сообщения);
   - если последнее сообщение БЕЗ медиа и caption непустой — форвард
     уходит как есть (без caption), а caption отправляется ОТДЕЛЬНЫМ
     обычным сообщением следом, через штатный `send(captionText)` (без
     `forwardFrom` — это уже не форвард, а новое сообщение
     пользователя).
   - при частичном фейле — `pendingForwardFailed` (текст ошибки под
     баннером), `pendingForward` НЕ сбрасывается и caption не теряется,
     повторный тап на Send просто повторяет попытку с начала батча.

6. Кнопка "Send" в композере теперь также появляется/активна, когда
   `pendingForward` установлен, даже если caption пустой (пустой
   caption = "переслать как есть").

tsc-clean. Commit 3f07fda.

Все пункты открытого вопроса "Форвард 2.0" из мастер-плана закрыты
(§6.219 + этот). Эмодзи-реакции по-прежнему визуальный плейсхолдер без
функционала — осознанно отложено Александром ("потом сделаем
функционал, не сейчас").

## 6.221 — Список чатів: стрілочка "переслано" перед прев'ю останнього повідомлення — 2026-09-05

Александр, референс-скриншот списку чатів: рядок "Mao" показує маленьку
стрілочку (↪) перед текстом прев'ю "А кроссы Асикс?" — ознака того, що
останнє повідомлення в чаті було переслано. В мобільному додатку такого
немає взагалі (перевірено: ні `chat_tile.dart`, ні
`chatReplyPreviewText` не читають `forwardFrom`), тож дизайн узятий
напряму зі скріншота, а не портований 1:1.

- `lib/a1/chat-schemas.ts` — `describeMessagePreview` тепер повертає
  `isForwarded` (`msg.forwardFrom?.object === "peer-user"`, той самий
  чек, що вже використовує `forwardToOneChat`/`originalAuthorId` в
  `app/chats/[chatId]/page.tsx`) поряд з `kind` — ортогонально виду
  повідомлення (пересланим може бути text/photo/voice/file/…).
- `app/api/chats/list/route.ts` — прокидає це як `previewForwarded`.
- `components/chat/chat-preview-line.tsx` — новий необов'язковий проп
  `isForwarded`; малює `ForwardIcon` (той самий гліф, що вже в пункті
  меню "Переслати" `message-actions-menu.tsx`, звідти й
  експортований) перед текстом/іконкою-лейблом для БУДЬ-ЯКОГО kind.
- `app/chats/page.tsx` та `components/chats-flyout.tsx` — прокидають
  `chat.previewForwarded` в `<ChatPreviewLine>`.

`resolveReplyPreview` (той самий компонент використовується і для
прев'ю "на що відповідаємо" в composer/bubble) свідомо НЕ зачеплено —
там мова про вид повідомлення, на яке відповідають, а не про факт
пересилання; проп `isForwarded` необов'язковий, тож той call-site
компілюється без змін.

tsc-clean. Commit 16d5820.

## 6.222 — Меню акаунта: міні-аватар у випадаючій панелі теж через CachedAvatar — 2026-09-05

Александр, скріншот меню акаунта на jobs.a1appp.com: маленький аватар
поруч з email/"Переглянути профіль" усередині випадаючої панелі —
"мини аватар справа в менюшке сделай тоже с подгрузкой через блюр".

`components/avatar-menu.tsx` мав ДВА різних аватар-сюрфейси: сама
кнопка-аватар у шапці вже отримала persistent Cache Storage-backed
`CachedAvatar` раніше тим самим днем ("кешировать вообще всё, якщо
воно хоча б 1 раз відкривалося"), але міні-аватар усередині самої
панелі (обидві гілки — з `profileUsername` і без нього) лишався на
звичайному `<img>` + `MEDIA_BLUR_STYLE` (одноразовий CSS-блюр, що
згасає один раз за вкладку, без кешу між навігаціями). Замінено на той
самий `CachedAvatar` (`size={32}`, під наявний `h-8 w-8`) в обох
місцях; невикористаний імпорт `MEDIA_BLUR_STYLE` прибрано.

tsc-clean. Commit a852794.

## 6.223 — Пілюля "Скопійовано" не перекриває шапку чату — 2026-09-05

Александр, скріншот: копіювання повідомлення біля самого верху чату
ставило пілюлю "Скопійовано" (§6.217, анкерена до скопійованої бабл-
строки) прямо поверх fixed-шапки, закриваючи ім'я співрозмовника —
"Опусти скопировано ниже имени".

`components/chat/copy-toast.tsx` раніше клемпив `top` лише по
`VIEWPORT_MARGIN` (14px від краю екрана) — достатньо, щоб не вилізти
за межі viewport, але нічого не заважало пілюлі осісти НАД шапкою,
якщо сама скопійована бабл-строка потрапляє в зону під fixed-
заголовком. Новий необов'язковий проп `minTop` піднімає нижню межу;
`app/chats/[chatId]/page.tsx` передає свій же живий `headerHeight`
(+12px запасу) — те саме значення, яким вже відбивається `paddingTop`
списку повідомлень від цієї самої шапки трохи вище.

`components/mini-chat-window.tsx` не займали — його власна шапка НЕ
fixed поверх списку повідомлень (звичайний `shrink-0` рядок у flex-
колонці), тож цей баг там фізично не відтворюється; пропущений
`minTop` зберігає стару поведінку (лише `VIEWPORT_MARGIN`) для нього.

tsc-clean. Commit dbc5889.

## 6.224 — Режим редагування: композер їде плавно, як і Reply — 2026-09-05

Александр, скрінкаст: вхід/вихід з режиму редагування повідомлення
різко смикав висоту композера, тоді як Reply вже давно їде плавно —
"сделай чтобы инпут филд ездил так же плавно как и при reply, а не
появлялся и исчезал резко".

`EditComposeBar` висів у JSX голим `{editingMessage && (...)}` без
анімаційної обгортки — `ReplyComposeBar` вже вирішує рівно ту саму
задачу через `displayedReplyTarget`/`replyRowGrown` (`grid-template-
rows` 1fr/0fr трюк + подвійний rAF, щоб навіть ВІДКРИТТЯ анімувалося,
а не тільки закриття — інакше свіжозмонтована нода не має "попереднього"
CSS-стану, з якого анімувати). Скопійовано той самий патерн 1:1:
`displayedEditingMessage`/`editRowGrown`, keyed на `editingMessage`.
`editingMessage` сам по собі як і раніше обнуляється МИТТЄВО при
cancel/успішному save (функціонально це не повинно чекати анімацію) —
`displayedEditingMessage` просто тримає `EditComposeBar` змонтованим
ще `EDIT_COLLAPSE_MS` (200мс), поки його обгортка їде назад до 0.

Сусідній `pendingForward`-банер тепер теж перевіряє
`!displayedEditingMessage` замість `!editingMessage`, щоб не вилізти
...
## 6.230 — Правильний відмінок у "Переслано від Вас" + повернули розмір іконки пам'яті — 2026-09-06

Aleksandr, Fix Tracker + скріншот: пересилка ВЛАСНОГО повідомлення (як у
Saved Messages) показувала "Переслано від Ви" — граматично невірно,
треба "Переслано від Вас". Помилка з'явилась разом із фіксом 6.226
("Переслано від …" тепер показує ваше ім'я): той фікс узяв
`YOU_LABEL_TEXT[lang]` — константу з називним відмінком ("Ви"/"You"),
призначену для підпису відправника (як ім'я: "Ти: привіт", шапка
фотов'ювера) — і підставив її туди, де насправді потрібен відмінок
ПІСЛЯ прийменника ("від X"/"from X"), а він у частини мов інший:

- укр./рос. "від"/"от" вимагають родовий: Ви → Вас, Вы → Вас;
- нім. "von" вимагає давальний: Du → dir;
- ісп. "de" — непрямий відмінок займенника: Tú → ti;
- пол. "od" вимагає родовий: Ty → Ciebie;
- en/fr/ptBR/zh не відмінюються — лишили як є.

Додано окрему константу `YOU_LABEL_FROM_TEXT: Record<Locale, string>`
(app/chats/[chatId]/page.tsx, одразу після `YOU_LABEL_TEXT`) саме під
цю конструкцію; підстановка у "Переслано від X"-лейблі (рядок ~5048)
перемкнута з `YOU_LABEL_TEXT[lang]` на `YOU_LABEL_FROM_TEXT[lang]`.
`YOU_LABEL_TEXT` сам лишився без змін — усі інші 3 місця, де він
використовується (шапка фотов'ювера, sender-лейбл, reply-quote
author), це підписи-іменники, називний відмінок там і був правильний.

Другим пунктом того ж скріншота — іконка Daily Uploads (`ChatStorageIcon`)
у попапі скрепки: була `h-4 w-4`, минулого разу (тикет "зроби в х2
більше") виросла до `h-8 w-8`, тепер за фідбеком "занадто величезна,
зменши на 50%" повернута назад на `h-4 w-4`.

Файли: `app/chats/[chatId]/page.tsx`.

Примітка з інфраструктури: під час цього фіксу сама `.git` папка
вперше зловила той самий баг iCloud "Optimize Mac Storage" (див. 6.228)
— `git status`/`git log`/`git commit` падали з "Resource deadlock
avoided" / Bus error на вивантажених `.git/index` і `.git/COMMIT_EDITMSG`.
Точкове підвантаження саме цих двох файлів через міст (а не всього
`node_modules`, як у 6.228 — тут файли маленькі) відновило git.
`git log`/`rev-list`, що йдуть по історії комітів, і далі падають —
частина старих loose-об'єктів у `.git/objects` теж вивантажена, але
для власне комміта поточної зміни це не завадило.

Commit 1c3fdf7.

## 6.231 — Закреп по ширині чату + плавні анімації появи/перемикання — 2026-09-06

Aleksandr, з веб-чек-листа Fix Tracker (додав самостійно, три пункти):
1. "Сузь закреп на ширину нашего чатового окна начиная от стрелки
   назад, заканчивая аватаром справа. Делай по этой ширине." —
   `PinnedMessageBanner` більше не задає власний `mx-3`; викликач
   (`app/chats/[chatId]/page.tsx`) огортає його в ТОЙ САМИЙ контейнер
   `mx-auto w-full max-w-[470px] px-4`, що й рядок шапки чату вище, тож
   краї баннера тепер збігаються зі стрілкою назад зліва й аватаром
   справа.
2. "При нажатии закрепа, делай чтобы он сверху появлялся плавно, с
   эффектом fade in, или плавной анимацией." — новий keyframe
   `.animate-pin-banner-in` (fade + невеликий зсув вниз, 0.22s) на
   кореневому `<div>` баннера.
3. "Так же, чтобы работали плавно все механики внутри него и тексты:
   'Відкріпити закріплене повідомленн[я]'" — новий `.animate-pin-
   content-fade` (0.15s, без зсуву) на обох внутрішніх станах баннера
   (звичайний рядок ↔ підтвердження "Відкріпити закріплене
   повідомлення?") і на обох варіантах правої кнопки (X ↔ "Відкріпити"),
   кожен з `key`, щоб React ретригерив анімацію при кожному перемиканні.

Обидва нових keyframes додані в `app/globals.css` тим самим сімейством,
що й інші попапи проєкту (`.animate-popover-up` тощо), і враховані в
`prefers-reduced-motion`.

Файли: `components/chat/pinned-message-banner.tsx`,
...
## 6.233 — Список нагадувань (Reminders list) — 2026-09-06

Референс-скріншоти показали iOS-подібний bottom sheet "Remind me" з
групами "Remind on July 8" / "Remind on July 9" (дата+час) — кожна
група з превʼю повідомлення, на яке поставлено нагадування. Це саме
той відкладений пункт, який §6.228 (коли запускалась фіча "Нагадати"
на кожне повідомлення) явно позначив як окрему задачу: "Список/
управление существующими напоминаниями (как в мобильном Reminders
bottom sheet) не будувалося — окрема задача при необхідності".

Ground-truth взято з мобільного застосунку (не вгадувалось):
- `reminders_modal_item.dart`: групування за датою+годиною (однакова
  календарна дата ТА однакова година — два нагадування за 5 хвилин
  одне від одного в одній групі, за 90 хвилин — вже в різних),
  сортування за зростанням.
- `api_constants.dart` + backend `.d.ts`: `messages.getReminderMessages`
  (`{ peerTo }` → голий масив `MessageReminder[]`, кожен —
  `{ local, message, scheduleAt }`) та `messages.deleteReminders`
  (`{ peerTo, ids: UInt[] }`, завжди викликається з одним id).
  Редагування — це той самий `messages.createReminder`, що й створення
  нового (мобільний `_showEditReminder` теж просто пересилає з новим
  `scheduleAt` для того самого id — окремого "update" методу нема).

Веб-специфічні адаптації (свідомо, не 1-в-1 клон мобільного):
- Рядок — компактне превʼю через `ChatPreviewLine`/`describeMessagePreview`
  (та сама пара, що вже використовує `PinnedMessageBanner`), а не повне
  переюзання бабла повідомлення — на вебі нема єдиного компонента для
  цього.
- Edit/Delete — завжди видимі іконки-кнопки замість мобільного
  long-press-контекстного меню.
- Видалення — тап-підтвердження (іконка кошика 3с перетворюється на
  червону кнопку "Видалити"), той самий патерн, що вже встановила кнопка
  "Відкріпити" в `PinnedMessageBanner`, замість другого підменю
  підтвердження в мобільного.
- Заголовки груп — та сама заокруглена пігулка, що й дата-роздільники в
  основній стрічці чату, у фіксованій темній палітрі модалки (як і
  `RemindModal`, з яким вона ділить візуальну мову).
- Точка входу — кнопка-дзвіночок (той самий glyph, що вже стоїть
  напроти пункту "Нагадати" в контекстному меню повідомлення —
  `RemindIcon` тепер експортований з `message-actions-menu.tsx` заради
  цього повторного використання). Спочатку поставив її в шапку чату
  поруч з аватаром навмання (явного референсу на розміщення не було, а
  точку входу в мобільному коді — Shortcuts tab — через масове
  вивантаження iCloud-файлів на момент розробки знайти не вдалося), але
  Aleksandr одразу після цього додав у Fix Tracker: "Я не понимаю куда
  сохраняется напоминание? Оно должно сохраняться возле инпут филда в
  виде такого звоночка, возле кота и при нажатии открывать попап с
  сообщениями" — перенесено в рядок іконок композера, одразу після
  `ChatCatFieldIcon` (commit 9183bc0).
- "Перейти до повідомлення" — перевикористаний вже наявний
  `handleJumpToPinnedMessage` (скрол + підсвітка), він і так був
  узагальненим, не привʼязаним лише до закріпленого повідомлення,
  просто раніше не мав іншого викликача.

Нові файли: `app/api/chats/reminders/list/route.ts`,
`app/api/chats/reminders/delete/route.ts`,
`components/chat/reminders-list-modal.tsx`.
Змінені: `lib/a1/chat-schemas.ts` (`ReminderItem`/`extractReminders`),
`components/chat/remind-modal.tsx` (опційні `initialScheduleAt`/
`initialLocal` для режиму редагування),
`components/chat/message-actions-menu.tsx` (`RemindIcon` експортовано),
`app/chats/[chatId]/page.tsx` (кнопка-тригер у шапці + рендер модалки).

Commit 4f2669b.

## 6.234 — Копіювання фото в буфер: виправлено мовчазну відмову — 2026-09-06

Aleksandr, з веб-чек-листа Fix Tracker: "Копирование фото пока не
работает" — регресія на фічі з §6.232 (комміт 4babc88), знайдена
буквально за хвилини після деплою.

Корінь: `navigator.clipboard.write()` у Safari (і дедалі частіше в
інших браузерах) працює надійно лише коли викликається СИНХРОННО в
межах user gesture (кліку). Стара версія спочатку робила `await fetch`
→ `await createImageBitmap` → `await canvas.toBlob` і лише ПОТІМ
викликала `write()` — на той момент браузер уже не вважав виклик
"прямою реакцією на клік" і мовчки відмовляв (порожній `.catch(() =>
{})` в `onCopy` ковтав помилку, тому нічого не відбувалось видимо).

Виправлення: `write()` тепер викликається синхронно прямо з обробника
кліку, а замість вже готового `Blob` йому передається ще незавершений
`Promise<Blob>` — конструктор `ClipboardItem` (`lib.dom.d.ts`) явно
підтримує `PromiseLike<string | Blob>` як значення, це задокументований
спосіб саме для такого "дані ще не готові" випадку. Сама логіка збірки
PNG (fetch/bitmap/canvas) не змінилась, лише винесена в окрему функцію
`buildPngBlobForClipboard`, чий Promise віддається в `ClipboardItem`
напряму, а не очікується заздалегідь.

Файли: `app/chats/[chatId]/page.tsx`.

Commit f426cd9.

## 6.235 — Закреп: плавне зникнення при відкріпленні — 2026-09-06

Aleksandr, з веб-чек-листа Fix Tracker: "відкріпити сделай, чтоби тоже
плавно исчезало как и появлялось" — §6.231 додав анімацію ПОЯВИ
баннера (`.animate-pin-banner-in`), але `handleTogglePin`'s
оптимістичне оновлення миттєво ставить `pinnedMessage` в `null` при
відкріпленні, а баннер рендерився простою умовою `{pinnedMessage &&
(...)}` — React видаляв DOM-вузол миттєво, жодна exit-анімація не
встигала відпрацювати.

Виправлення: новий стан `displayedPinnedMessage`, що відстає на один
крок від реального `pinnedMessage` — той самий трюк, що вже
використовує `displayedReplyTarget` (схлопування reply-плашки) в
цьому ж файлі. При закріпленні оновлюється миттєво; при відкріпленні
тримає останнє повідомлення `PIN_BANNER_EXIT_MS` (200мс), поки грає
новий keyframe `.animate-pin-banner-out` (дзеркальний до `-in`), а
потім справді очищається. Перемикання чату скидає обидва стани
миттєво (не справжнє відкріплення — анімувати нема чого).

Файли: `app/chats/[chatId]/page.tsx`, `app/globals.css`.

Commit a55a0f7.

## 6.236 — Заголовок пересланого повідомлення: імʼя знизу + аватарка — 2026-09-06

Aleksandr, 2 референс-скріншоти (наш живий "Переслано від X" в один
рядок vs. заголовок пересланого повідомлення у WhatsApp): "«Переслано
від» имя пиши снизу и ставь аватарку как в Телеге, ее еле видно,
поэтому должен быть очень легкий вес".

Було одним рядком ("Переслано від X" через інтерполяцію рядка в `<T>`,
через що це не могло бути справжнім розділенням лейбл+імʼя по
локалях). Тепер: лейбл "Переслано від" — окремим рядком, імʼя
відправника — другим рядком нижче, з маленькою аватаркою перед ним
(`contactSummaries[...].avatarUrl`, або `myAvatarUrl` при пересиланні
власного повідомлення, з тим самим фолбеком на
`pickDefaultCatAvatar`, що й усі інші аватарки в цьому файлі). "еле
видно" — саме про вагу аватарки: залишена крихітною (14px) і на 70%
прозорості замість повноцінного вигляду, який мають усі інші аватарки
цього файлу — так само неяскраво, як це виглядає в самому Telegram.

Файли: `app/chats/[chatId]/page.tsx`.

Commit 22413c6.

## 6.237 — Стікери/GIF/емодзі-панель + реакції: дослідження і витягнута анімація ❤️ — 2026-09-06

Олександр попросив велику фічу у два блоки: (1) панель стікерів/GIF/емодзі
під котиком у полі вводу (як у Telegram, з перемикачем GIFs/Stickers/Emoji),
(2) робочі реакції на повідомлення (ПКМ на десктопі, тап на мобільному,
тогл при повторному натисканні тієї ж реакції). Це — не готова фіча, а
етап дослідження: розібратися, як усе влаштовано в мобільному додатку і
бекенді, витягнути що можна, зробити майстер-план.

Результати дослідження (мобільний код + бекенд, джерело істини):
- GIF-пошук — це НАШ бекенд (`media.globalSearch`, `documentType: gif`), а
  не сторонній Tenor/Giphy. GIF-документи мають ту саму форму, що й звичайні
  медіа-вкладення — можна перевикористати наявну проксі-інфраструктуру.
- Стікери — справжні Telegram `.tgs` файли (gzip-стиснутий Lottie JSON),
  організовані в набори (sticker sets) з бекенду, з кешем на клієнті.
- Реакції: `reactions` масив на повідомленні вже приходить з бекенду, але
  зараз не парситься на вебі (`RawMessageSchema` його не ловить — губиться
  в catchall). Правило "одна реакція на юзера" — це клієнтська логіка
  (`dedupeReactionsToLatestPerUser`), бекенд дозволяє кілька одночасно.
  Методи: `messages.addReaction` / `messages.deleteReaction`.
- Анімація реакції: тільки ❤️ має особливу Lottie-анімацію (спалах у
  центрі), решта реакцій — легкі конфеті-частинки. Анімацію вдалося
  витягнути: `assets/tgs/heart_reaction.tgs` → розпакували gzip →
  `public/animations/heart_reaction.json` (готовий Lottie JSON, вебка вже
  вміє рендерити такі через `lottie-web`/`components/lottie-player.tsx`).
  Commit eeac07f.
- Знайдена розбіжність: наш поточний (нефункціональний) плейсхолдер
  реакцій у `message-actions-menu.tsx` використовує 😄 замість 😁 як 7-му
  емодзі — на мобільному стандартний набір: 👍 👎 ❤️ 🔥 🥰 👏 😁.
- Два файли мобільної панелі стікерів (`gif_panel_tab.dart`,
  `emoji_panel_tab.dart`) не вдалося прочитати через технічне обмеження
  (задокументовано, не критично для плану — головна логіка видна з
  батьківського `sticker_panel.dart`).

Це підготовчий крок. Повний технічний розбір і покроковий план — в
окремому файлі `STICKERS_AND_REACTIONS_PLAN.md` в корені репозиторію.
Саму фічу починаємо робити окремим кроком, після узгодження з Олександром.

Файли: `public/animations/heart_reaction.json` (новий), `STICKERS_AND_REACTIONS_PLAN.md` (новий, довідник).

Commit eeac07f.

## 6.238 — Реакції на повідомлення: базова реалізація (дані + API + тогл) — 2026-09-06

Олександр підтвердив усі відкриті питання з §6.237 і дав добро робити
все одразу (стікери/GIF/емодзі-панель + реакції, реакції на всі типи
контенту — фото, файли, розрахункові повідомлення). Почав з реакцій —
найбільш опрацьована частина.

Що зроблено:
- Схема даних: `reactions` тепер парситься на кожному повідомленні
  (раніше губилося в catchall). Логіка "одна реакція на юзера, остання
  перемагає" перенесена з мобільного (dedupeReactionsToLatestPerUser).
- Два нових API-роути: `/api/chats/reaction/add` і `.../reaction/delete`
  — виклик `messages.addReaction`/`messages.deleteReaction` на бекенді.
- Ряд реакцій у купертіно-меню (правий клік) тепер реально працює —
  раніше був візуальним плейсхолдером. Виправлено розбіжність з
  мобільним набором емодзі (7-й був не той).
- Під кожним повідомленням (текст, фото, файл, стікер, контакт,
  розрахунок — одна спільна точка вставки для всіх типів) з'явився ряд
  чіпів реакцій: емодзі + аватарка співрозмовника (чат один-на-один,
  тому реакторів завжди максимум двоє). Клік по чіпу — тогл (додає свою
  реакцію/забирає, якщо вона вже стоїть).
- При встановленні реакції ❤️ програється витягнута раніше Lottie-
  анімація (heart_reaction.json, §6.237).

Що ще не зроблено (в STICKERS_AND_REACTIONS_PLAN.md): конфеті-ефекти
для інших емодзі (окрім ❤️), одиночний тап на мобільних (зараз лише
правий клік на десктопі), сама панель стікерів/GIF/емодзі (блок 1 —
наступний крок).

Файли: `lib/a1/chat-schemas.ts`, `components/chat/message-actions-menu.tsx`,
`app/chats/[chatId]/page.tsx`, `app/api/chats/reaction/add/route.ts` (новий),
`app/api/chats/reaction/delete/route.ts` (новий).

Commit e3f1867.

## 6.239 — Блок 1: панель стікерів/GIF/емодзі + реальний рендер стікерів — 2026-09-06

Олександр: "Давай делать" — продовження після §6.238. Реалізовано блок 1
повністю (без поетапності, як він і просив: "делаем всё сразу").

Що зроблено:
- Панель відкривається по кліку на іконку кота в барі вводу (раніше
  суто декоративна, без onClick). Три вкладки внизу (GIF/Стікери/
  Emoji), стікери — стартова вкладка, стрілка закриття зверху.
- Бекенд-контракти підтверджено з реального коду chat-server/
  media-server (не вгадано): `messages.getAllStickers`,
  `messages.getRecentStickers` (chat-server) і `media.globalSearch`
  (media-server — інший мікросервіс, але той самий публічний API-базовий
  URL, окремої маршрутизації на клієнті не знадобилось). Три нові
  тонкі проксі-роути за тим самим патерном, що й у реакцій.
- Вкладка Emoji: власний підібраний набір емодзі по категоріях
  (`lib/a1/emoji-data.ts`) замість нової залежності — покриває всі
  категорії з референсів, легко розширити пізніше.
- Вибір стікера/GIF надсилає його одразу (без окремого кроку
  "прикріпити, потім Відправити") — жодного завантаження не треба,
  обидва вже є готовими MediaDocument на бекенді, тож
  `sendMediaDocument()` іде прямо в існуючий `/api/chats/send` з
  готовим `fileReference`.
- Закрито прогалину з 2026-09-03 (плейсхолдер-чіп "Стікер" замість
  реального зображення, явно позначено як "окремий наступний крок"):
  `components/chat/tgs-sticker.tsx` розпаковує .tgs (gzip) прямо в
  браузері (`DecompressionStream`) і рендерить через lottie-web, зі
  старим чіпом як фолбек при помилці. Реальний стікер тепер теж без
  підкладки бульбашки (новий прапорець `isStickerOnly`, той самий
  підхід, що вже був у привітального стікера).

Що ще не зроблено: власний UI для реакції під стікером з ім'ям
(Олександр обіцяв прислати окремо), точна відповідність пікселям
референсів (панель зроблена функціонально, не звірялась з Figma).

Файли: `lib/a1/media-panel-schemas.ts` (новий), `lib/a1/emoji-data.ts`
(новий), `components/chat/tgs-sticker.tsx` (новий),
`components/chat/media-picker-panel.tsx` (новий),
`app/api/chats/stickers/sets/route.ts` (новий),
`app/api/chats/stickers/recent/route.ts` (новий),
`app/api/chats/gifs/search/route.ts` (новий), `app/chats/[chatId]/page.tsx`.
