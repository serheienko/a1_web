...
- Точка входу — нова кнопка-дзвіночок у шапці чату, поруч з аватаром
  (той самий glyph, що вже стоїть напроти пункту "Нагадати" в
  контекстному меню повідомлення — `RemindIcon` тепер експортований з
  `message-actions-menu.tsx` заради цього повторного використання).
  Явного референсу на розміщення тригера не було — рішення прийнято
  самостійно, оскільки в мобільному коді точку входу (Shortcuts tab)
  через масове вивантаження iCloud-файлів на момент розробки знайти не
  вдалося.
