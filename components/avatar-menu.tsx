...
export function AvatarMenu() {
  ...
  if (!email) {
    return (
      ...
      // Александр, 19.09.2026, третий заход: «дело именно в расположении
    // иконок по вертикали». И он был прав, а я дважды мимо. Замеры на
    // живой странице: аватар и кнопка приложения оба 44px, но центр
    // аватара стоял на 3px выше центра всей остальной шапки.
    //
    // Причина -- строчный контекст. <button> по умолчанию inline-block,
    // а эта обёртка была обычным блоком, поэтому кнопка садилась на
    // базовую линию строки, и под ней оставалось место под свисающие
    // хвосты букв: обёртка получалась 49.3px при кнопке в 44px. По
    // центру выравнивалась ОБЁРТКА, а кнопка внутри неё прижималась к
    // верху -- отсюда и смещение.
    //
    // flex + items-center убирает строку целиком: обёртка становится
    // ровно по высоте кнопки. Проверено в браузере -- разница центров
    // стала 0.00px.
