...
## Ответы Александра (2026-09-06) и статус

1. Порядок реализации блока 1 (стикеры/GIF/эмодзи) — делаем всё сразу,
   без поэтапного стейджинга.
2. Клик по уже показанному чипу реакции на десктопе — нужен, тогл
   (убирает свою реакцию / ставит, если её ещё нет). **Сделано.**
3. Реакции для расчётных сообщений — да, делаем. Плюс уточнение: реакции
   нужны для ВСЕХ типов контента в чате (фото, файлы, стикеры, контакты,
   расчёты), не только текст. **Сделано** — общая точка вставки в
   `app/chats/[chatId]/page.tsx` рендерит `ReactionsBar` под любым типом
   сообщения одним кодом, без дублирования на каждый тип.
4. Про нативную оптимизацию рендера стикеров — ответа не было, не
   блокирует старт, вернёмся к этому при реализации блока 1.

Доп. деталь из скриншотов реального чата: чип реакции показывает
эмодзи + аватарку среагировавшего собеседника (не число), для
сообщений разных типов (текст, стикер, шаренный контакт, расчёт).
Реализовано с допущением "чат один-на-один → максимум два реактора"
(своя реакция подсвечивается заливкой чипа, чужая — аватаркой рядом
с эмодзи). Для стикеров Александр упомянул отдельный вариант UI (чип
"живёт под стикером с именем"), пришлёт скриншоты отдельно — пока не
реализовано, чип рендерится тем же общим компонентом.

**Статус блока 2 (реакции): базовая версия сделана** (commit e3f1867,
PLAN.md §6.238). Не сделано: конфетти-эффекты для не-❤️ эмодзи
(reaction_confetti.dart's `_Recipe` за каждый эмодзи — координаты уже
прочитаны, осталось перенести), одиночный тап на мобильных (сейчас
только ПКМ на десктопе), сама панель стикеров/GIF/эмодзи (блок 1).

**Статус блока 1 (стикеры/GIF/эмодзи-панель): сделано** (commit
90e598c, PLAN.md §6.239). Панель открывается по клику на иконку кота,
три вкладки (GIF/Стикеры/Emoji, стикеры — стартовая), три новых
API-роута (`getAllStickers`, `getRecentStickers`, `media.globalSearch`),
выбор стикера/GIF отправляет его сразу через существующий
`/api/chats/send`. Заодно закрыт долг с 2026-09-03: реальный рендер
.tgs-стикеров (гzip-декод в браузере + lottie-web) вместо плейсхолдер-чипа
— `components/chat/tgs-sticker.tsx`.

Не сделано: отдельный UI для реакции под стикером с именем (Александр
пришлёт референсы отдельно), точная сверка вёрстки с Figma (панель
сделана функционально по описанию скриншотов, не пиксель-в-пиксель),
нативные оптимизации рендера стикеров (см. открытый вопрос №4 ниже —
без ответа, не блокировало старт).
