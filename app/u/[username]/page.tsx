...
function levelBar(level: number, max: number) {
  ...
  return (
    <div className="h-1.5 w-full max-w-[160px] overflow-hidden rounded-full bg-neutral-100">
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-10 sm:py-16">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl font-semibold text-neutral-900 sm:text-2xl">{profile.fullName}</h1>
          <p className="text-sm text-neutral-500">@{profile.username}</p>
        </div>
      </div>

      {profile.profileTitle && <p className="mt-4 text-base text-neutral-700">{profile.profileTitle}</p>}
      ...
      {profile.bio && <p className="mt-6 whitespace-pre-wrap text-neutral-700">{profile.bio}</p>}
      ...
      {profile.companies.length > 0 && (
        <section className="mt-8">
          <h2 className="text-sm font-medium uppercase tracking-wide text-neutral-400">Опыт работы</h2>
            ...
            {profile.companies.map((company, i) => (
              <div key={`${company.name}-${i}`}>
                ...
                {company.positionDescription && (
                  <div className="text-sm text-neutral-600">{company.positionDescription}</div>
                )}
                {company.description && <div className="mt-1 text-sm text-neutral-500">{company.description}</div>}
              </div>
            ))}
          </div>
        </section>
      ...
      {profile.education.length > 0 && (
        <section className="mt-8">
          <h2 className="text-sm font-medium uppercase tracking-wide text-neutral-400">Образование</h2>
      ...
      {profile.skills.length > 0 && (
        <section className="mt-8">
          <h2 className="text-sm font-medium uppercase tracking-wide text-neutral-400">Навыки</h2>
          <div className="mt-3 flex flex-col gap-2">
            {profile.skills.map((skill) => (
              <div key={skill.value} className="flex items-center gap-3">
                <span className="w-32 shrink-0 text-sm text-neutral-700">{skill.value}</span>
      ...
      {profile.languages.length > 0 && (
        <section className="mt-8">
          <h2 className="text-sm font-medium uppercase tracking-wide text-neutral-400">Языки</h2>
          <div className="mt-3 flex flex-col gap-2">
            {profile.languages.map((lang) => (
              <div key={lang.value} className="flex items-center gap-3">
                <span className="w-32 shrink-0 text-sm text-neutral-700">{lang.value.toUpperCase()}</span>
